...
var setup = function () {
    ...
    context.emptyMessage = this.data.emptyMessage || this.data.settings.emptyMessage || 'Nothing to show';
    context.childrenTemplateName = this.data.childrenTemplateName || this.data.settings.childrenTemplateName;
    context.childrenFooterTemplateName = this.data.childrenFooterTemplateName || this.data.settings.childrenFooterTemplateName;
    context.isChildrenClosed = this.data.isChildrenClosed || this.data.settings.isChildrenClosed || false;
...
Template.reactiveTable.events({
    ...
    'click .reactive-table .js-toggle-node': function(event, template) {
        var row = template.$(event.target).parents('tr'),
            toggler = row.find('.js-toggle-node'),
            child = row.next('tr.child');

        while (child.length) {
            child[row.attr('data-closed') === 'true' ? 'removeClass' : 'addClass']('hidden');

            child = child.next('tr.child');
        }

        row.attr('data-closed', row.attr('data-closed') === 'true' ? 'false' : 'true');

        toggler.removeClass('fa-caret-down').removeClass('fa-caret-right').addClass(row.attr('data-closed') === 'true' ? 'fa-caret-right' : 'fa-caret-down');
    }
});
