...
Template.reactiveTable.events({
    ...
    'click .reactive-table .js-toggle-node': function(event, template) {
        ...
        while (child.length) {
            child[row.data('closed') === 'true' ? 'removeClass' : 'addClass']('hidden');

            child = child.next('tr.child');
        }

        row.data('closed', row.data('closed') === 'true' ? 'false' : 'true');

        toggler.removeClass('fa-caret-down').removeClass('fa-caret-right').addClass(row.data('closed') === 'true' ? 'fa-caret-right' : 'fa-caret-down');
    }
});
